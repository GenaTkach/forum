...
public class AuthenticationFilter implements Filter {
	...
	if (checkEndPoint(request.getMethod(), request.getServletPath())) {
	    ...
	    String sessionId = request.getSession()
		    .getId();
	    UserAccount userCookies = sessionService.getUser(sessionId);
	    
	    // Если такой сессии не существует
	    if(userCookies == null) {
	    ...
	    request = new WrappedRequest(request, userCookies.getLogin());
	    User user = User.builder()
		    .userName(userCookies.getLogin())
		    .password(userCookies.getPassword())
		    .roles(userCookies.getRoles())
		    .build();
